document.addEventListener('DOMContentLoaded', function() {
    ...
    function startGame() {
        ...
        clearContent();
        addParagraph('Cher Gueux de 2023,<br><br>En cette nuit de Noël 2025, deux ans après votre évasion légendaire...<br>Le Château de Satanae vous appelle une dernière fois. Des récompenses ? Des pièges ? De la nostalgie ?<br><br>Venez, si vous osez revivre l\'horreur festive.<br><br>Signé,<br>Dr. Amadeus 🧛‍♂️❄️🩸');
    ...
    function badEnd() {
        clearContent();
        addParagraph('Tu chiffonnes la lettre... et rates l\'aventure. Joyeux Noël solitaire, gueux pathétique ! 🎄😭');
    ...
    function choice1() {
        ...
        clearContent();
        addParagraph('Le château se dresse, enneigé et menaçant sous la lune rouge.<br><br>Entre ton nom de gueux d\'Halloween 2023 pour que mes chauve-souris te reconnaissent :');
    ...
    function checkName() {
        ...
        clearContent();
        if (input === "amadeus") {
            addParagraph('#JyCroisMoyen<br>Tu n\'es pas moi, imposteur ! Mes chauve-souris t\'attaquent. 💀');
            addButton("Réessayer", choice1);
            return;
        }
        if (validNames.includes(input)) {
            playerName = input.charAt(0).toUpperCase() + input.slice(1);
            addParagraph('Ah, ' + playerName + '... La nostalgie est palpable. Mes yeux (les chauve-souris) te reconnaissent.');
            ...
            addParagraph('Nom inconnu... Mes chauve-souris te snobent. Réessaie, gueux oublié.');
    ...
    function cellsScene(choice) {
        ...
        addParagraph(introText + '<br><br>Tu te réveilles à l\'étage -6, enchaîné avec les autres gueux. Les torches crépitent, les chaînes grincent...<br>L\'aventure recommence vraiment maintenant, ' + playerName + '.');
        addButton("Continuer vers le -5 (Edge Lord incoming...)", nextScene);
    }

    function nextScene() {
        clearContent();
        addParagraph('À suivre bientôt : Combat contre l\'Edge Lord qui pue la Monster et RedBull, le labo avec potions wtf et cape emo stylée, le journal intime du crush toxique sur Satanae, Ganondorf charcuté par Link, la cuisine heal avec chien en armure amadoué, les quartiers privés, le big fight hall, et le boss secret "La Source du Mal" mi-femme mi-IA cosmique !<br><br>Joyeux Noël PAS normal 2025, les gueux ! 🧛‍♂️🎄🩸');
